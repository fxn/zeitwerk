...
class TestUnload < LoaderTest
  ...
  test "unload clears explicit namespaces associated" do
    ...
    with_files(files) do
      ...
      assert Zeitwerk::Registry.namespaces.registered?(crefM) == la

      lb = new_loader(dirs: "b")
      crefX = Zeitwerk::Cref.new(Object, :X)
      assert Zeitwerk::Registry.namespaces.registered?(crefX) == lb

      la.unload
      assert_nil Zeitwerk::Registry.namespaces.registered?(crefM)
      assert Zeitwerk::Registry.namespaces.registered?(crefX) == lb
    end
  end
